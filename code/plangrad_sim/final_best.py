"""Final headline comparison: Stage-1 vs Stage-2 under the BEST planner
config (alpha=0.1, Hp=15, a_max=20), held-out 2500-3000, n=200, seed
12345. Shows ADE halved by Stage-2, with safety from the tuned planner.
Writes BEST.txt with flush.
"""
import torch
...
if __name__ == "__main__":
    set_seed(12345)
    open(OUT, "w").close()
    w("BEST: planner alpha=%.1f Hp=%d a_max=%.0f, n=%d, seed 12345"
      % (ALPHA, HP, AMAX, N))
    cr1, sep1, ade1 = evaluate(load("stage1_full.pt"))
    w("Stage-1   CR=%.1f%%  minSep=%.1f m  ADE=%.2f m" % (cr1, sep1, ade1))
    cr2, sep2, ade2 = evaluate(load("stage2_final.pt"))
    w("Stage-2   CR=%.1f%%  minSep=%.1f m  ADE=%.2f m" % (cr2, sep2, ade2))
    w("delta     CR %+.1f pts  minSep %+.1f m  ADE %+.2f m"
      % (cr2 - cr1, sep2 - sep1, ade2 - ade1))
