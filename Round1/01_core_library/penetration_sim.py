    ...
    def run(self, p_equip, horizon_steps=600, warmup=100):
        ...
        for step in range(horizon_steps):
            ...
            for ego in live:
                ...
                out = abs(ego.state[1]) > self.Hw * 3 or ego.travel_steps > \
                    self.free_flow_steps * 4
                if reached:
                    ego.done = True; ego.passed = True; finished.append(ego)
                elif out:
                    ego.done = True; ego.passed = False; finished.append(ego)
