...
class PenetrationCorridor:
    ...
    def run(self, p_equip, horizon_steps=600, warmup=100):
        ...
        # RIGHT-CENSORING AUDIT: agents still in the corridor at sim end
        # (never reached exit, never went lateral, never hit the time cap).
        # They were previously dropped silently; count them here.
        still_in = [a for a in agents if not a.done and a.spawn_step >= warmup]
        n_censored = len(still_in)
        # count agents that entered after warmup (steady-state measurement)
        measured = [a for a in finished if a.spawn_step >= warmup]
        m = self._metrics(measured, horizon_steps - warmup)
        m["n_censored_in_corridor"] = n_censored
        m["n_finished_postwarmup"] = len(measured)
        return m
