...
def agg(runs, key):
    ...
    return (np.nanmean(v), np.nanstd(v), np.nanmean(t), np.nanstd(t),
            np.nanmean(d), np.nanstd(d), int(np.sum(ns)))
    ...
    for p in ps:
        ...
        for rep in range(args.reps):
            ...
            m = sim.run(p_equip=p, horizon_steps=args.horizon,
                        warmup=args.warmup)
            runs.append(m)
        cra, crs, thr, thrs, dly, dlys, na = agg(runs, "all")
        cre, cres, thre, thres, dlye, dlyes, nae = agg(runs, "equipped")
        cru, crus, thru, thrus, dlyu, dlyus, nau = agg(runs, "unequipped")
        w("")
        w("### p = %d%% equipped" % int(p * 100))
        w("  ALL        : CR=%5.1f+-%4.1f%%  Thr=%5.2f+-%4.2f/min  Delay=%5.1f+-%4.1fs  (n=%d)"
          % (cra, crs, thr, thrs, dly, dlys, na))
        w("  EQUIPPED   : CR=%5.1f+-%4.1f%%  Thr=%5.2f+-%4.2f/min  Delay=%5.1f+-%4.1fs  (n=%d)"
          % (cre, cres, thre, thres, dlye, dlyes, nae))
        w("  UNEQUIPPED : CR=%5.1f+-%4.1f%%  Thr=%5.2f+-%4.2f/min  Delay=%5.1f+-%4.1fs  (n=%d)"
          % (cru, crus, thru, thrus, dlyu, dlyus, nau))
