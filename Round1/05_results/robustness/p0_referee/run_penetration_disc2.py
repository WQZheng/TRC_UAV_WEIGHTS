    ...
    for p in ps:
        ...
        for rep in range(args.reps):
            ...
            m = sim.run(p_equip=p, horizon_steps=args.horizon,
                        warmup=args.warmup)
            runs.append(m)
        cra, crs, thr, thrs, dly, dlys, na, npa, compa, ntoa, nlaa, dcena = agg(runs, "all")
        cre, cres, thre, thres, dlye, dlyes, nae, npe, compe, ntoe, nlae, dcene = agg(runs, "equipped")
        cru, crus, thru, thrus, dlyu, dlyus, nau, npu, compu, ntou, nlau, dcenu = agg(runs, "unequipped")
        w("")
